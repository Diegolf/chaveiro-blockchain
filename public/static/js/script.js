                    ...
                    getChave(chvs[i], (err, result) => {
                        if(err){
                            ...
                            $('.remove_chave').off('click').on('click', function() {
                                ...
                                removeChave(chv, function (err, result) {
                                    if(err){
                                        ...
                                        toast2('Remover Chave, aguardando confirmação: <span>'+((TRANSACTION_CHECK+800)/1000)+'</span> ',TRANSACTION_CHECK);
    ...
    $('#nova_chave').click(() => {
        ...
        adicionarChave(destino, usuario, senha, (err, result) => {
            if(err){
                ...
                toast2('Nova Chave, aguardando confirmação: <span>'+((TRANSACTION_CHECK+800)/1000)+'</span> ',TRANSACTION_CHECK);
...
function toast2(mensagem, tempo){
    ...
    x.append(toast_div.replace('#NUM',id).replace('#TOASTMESSAGE',mensagem));
    timerToast2($('.toast-message[numero="'+id+'"]'));
    setTimeout( function () { 
        $('.toast-message[numero="'+id+'"]').remove();
        if (x.html() == ""){
            x.css('visibility','hidden');
...
function timerToast2(toast_div){
    let val = parseInt(toast_div.children().html()) - 1;
    toast_div.children().html(val);
    if (val != 0){
        setTimeout(() => {
            timerToast2(toast_div)
        }, 1000);
        // TODO window.list[id] = // remover todos os timeouts quando a pessoa der logout
    }
}
